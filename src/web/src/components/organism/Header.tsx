  ...
  return (
    ...
    <div className="lg:px-96 md:px-48 overflow-x-clip scrollbar-hide px-16 py-24 grid lg:grid-cols-12 grid-cols-2 items-center w-full z-50 relative">
      ...
      }
      {
        ...
        <div className={`col-span-1 ${props.showNavLinks === false && "col-span-4"} flex cursor-pointer justify-end`}>
        <button
      ...
      <div className={`absolute z-30 rounded-2xl shadow-lg lg:hidden top-[90px] lg:w-full w-[85%] transition-all delay-100 ease-in-out ${showMenu ? "right-[8px]  mx-auto left-0 " : "right-[-90%]"}  px-16 py-[31px] h-max gap-11 bottom-0 bg-N00 flex flex-col justify-between`}>
      {
        props.showNavLinks ?
        <NavLink />: null
      }
      {
        props.showApplyButton?
        <div className={`col-span-4 flex cursor-pointer justify-end`}>
          <button
      ...
      <div className="col-span-1 lg:hidden flex justify-end cursor-pointer transition-all delay-150 ease-in-out">
        ...
        :
        <Image src={MenuIcon} alt="menu-icon" className="h-auto w-auto" 
        onClick={() => setShowMenu(!showMenu)}/>
        }
