...
pkill -f "celery -A map worker -n prod-celery --concurrency=2 --prefetch-multiplier=1 --task-acks-late=True --loglevel=info --detach"
...
celery -A map worker -n prod-celery --concurrency=2 --prefetch-multiplier=1 --task-acks-late=True --loglevel=info --detach 
